...
@interface MySecondHandVC ()<UITableViewDataSource,UITableViewDelegate, MyFreeDelegate>{
...
#pragma 我的闲置的代理方法
- (void)ShowMessage{
    UIAlertController *alertVC = [UIAlertController alertControllerWithTitle:@"展示规则" message:@"每个二手商品都有固定的在线展示时间(90天),如果连续90天内业主没有卖出或手动续期，系统会自动进行下架操作" preferredStyle:UIAlertControllerStyleAlert];
...
- (void)deleteMyFree:(NSString *)MyFreeGoodsId{
    NSString *urlString;
    if (_isHouse) {
         urlString = deleteMyFreeHouse;
    }else{
         urlString = deleteMyFreeGoods;
    }
    
    NSMutableDictionary *mudic = [NSMutableDictionary dictionary];
    [mudic setObject:MyFreeGoodsId forKey:@"id"];
    [[HYHttp sharedHYHttp] POST:urlString parameters:mudic success:^(id  _Nonnull responseObject) {
//        NSLog(@"%@--", responseObject);
        if ([responseObject[@"success"] intValue]) {
            [ShowMessage showTextOnly:responseObject[@"obj"] messageView:self.view];
            [self reloadShowsPage];
        }
...
- (void)editorMyFree:(NSString *)MyFreeGoodsId{
    ...
    NSLog(@"%@", MyFreeGoodsId);
    if (_isHouse) {
        nextVC.GoodsClass = @"房屋出租";
    }else{
        nextVC.GoodsClass = @"闲置物品";
    }
    nextVC.MyFreeGoodsId = MyFreeGoodsId;
    nextVC.titleNameStr = @"编辑闲置";
...
- (void)initTabView{
    ...
    [self.tableView registerNib:[UINib nibWithNibName:NSStringFromClass([MySecondTableViewCell class]) bundle:nil] forCellReuseIdentifier:@"myCell"];
}
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    MySecondTableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:@"myCell"];
    if (!cell) {
        cell = [[MySecondTableViewCell alloc]initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"myCell"];
